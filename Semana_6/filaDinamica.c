...
PONT retornarPrimeiro(FILA* f, REGISTRO *ch){
  if (f->inicio != NULL) *ch = f->inicio->reg;
  return f->inicio;
} /* retornarPrimeiro */
...
REGISTRO inserirNaFila2(FILA* f, char tipo) {
  PONT novo = (PONT) malloc(sizeof(ELEMENTO));
  if(f->fim == NULL) {
    novo->reg.chave = 1;
  } else {
      novo->reg.chave = f->fim->reg.chave + 1;
  }
  novo->reg.tipo = tipo;
  novo->prox = NULL;
  if (f->inicio==NULL){
...
void exibirFila(FILA* f){
  ...
  printf("Fila: \" ");
  while (end != NULL){
    printf("%c%d ", end->reg.tipo ,end->reg.chave); // soh lembrando TIPOCHAVE = int
    end = end->prox;
  }
  printf("\"\n");
} /* exibirFila */ 
